...
<body>
    ...
    <form action="" method="post">
        <div style="text-align:center;font-size: 20px;margin-bottom: 10px;font-weight:bold;color:#3699FF;"
        ...
        <div class="inputGroup inputGroup2">
            <input type="password" id="password" name="pass" class="password" maxlength="30" required
        ...
        if (isset($_POST['btn'])) {
            ...
            $stmt = $conn->prepare("SELECT Ma_nv, ID_bophan,Hoten FROM nhan_vien WHERE Ma_nv = ? AND password = ?");
            $stmt->bind_param("ss", $user, $pass);
            $stmt->execute();
            $stmt->store_result();
            if ($stmt->num_rows > 0) {
                $stmt->bind_result($user, $role, $hoten);
                $stmt->fetch();

                // Đặt thông tin người dùng vào phiên
                $_SESSION['Ma_nv'] = $user;
                $_SESSION['ID_bophan'] = $role;
                $_SESSION['Ho_ten'] = $hoten;
                $login_cookie_name = "login";
                $login_cookie_value = $user;
                ...
                if ($role == 1) {
                    ...
                    header("Location: employees/pages/thong_ke.php");
                }
                exit;
            } else {
                echo ' <script>
